  ...
  return (
    <div className="flex flex-col flex-1 p-4 sm:p-6 mt-4 w-full text-base text-white rounded-lg border border-solid bg-white bg-opacity-30 border-white border-opacity-20">
      ...
      {allMessages.map((message, index) => (
        <ChatMessage 
        key={index} 
        avatar={message.isUser ? "src/assets/images/avatar-1.png" : "src/assets/images/MetaHuman_SA_FM1.png"} 
        message={message.text} 
        isUser={message.isUser} 
        />
      ))}
      {isLoading && <ChatMessage isLoading={isLoading} avatar="src/assets/images/MetaHuman_SA_FM1.png" isUser={false}/>}
      {streamingResponse && <ChatMessage avatar="src/assets/images/MetaHuman_SA_FM1.png" message={streamingResponse} isUser={false}/>}
        <div ref={messagesEndRef} />
      </div>
      <style jsx>{`
        .custom-scrollbar {
